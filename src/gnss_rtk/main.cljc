(ns gnss_rtk.main
  "Kotodama WASM entrypoint for the Gnss_rtk clean-room actor (L4) — Clojure port.
  ...
  facts (`gnss_rtk.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "gnss_rtk")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"gnss_rtk.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "gnss_rtk-compat" :tier tier :entities entities} 200])
...
(defn start! [] :gnss_rtk-compat/ready)
